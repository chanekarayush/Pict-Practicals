...
def isSafe(pos:tuple, board: list)->bool:
    ...
    # Check ROW
    for r in range(ROW):
        if board[r][col].lower() == 'q':
            return False
    # Check COL 
    for c in range(COL):
        if board[row][c].lower() == 'q':
            return False 
    ...
    for r, c in zip(range(row, ROW), range(col, -1, -1)):
        if board[r][c].lower() == 'q':
            return False

    # Check Upper Right Diagonal
    for r, c in zip(range(row, -1, -1), range(col, COL)):
        if board[r][c].lower() == 'q':
            return False

    # Check Lower Right Diagonal
    for r, c in zip(range(row, ROW), range(col, COL)):
        if board[r][c].lower() == 'q':
            return False
    return True


def backtrack_NQueens(board: list, solutions: list, queens: int, col: int):
    if queens == 0:
    ...
    for r in range(len(board)):
        ...
        if isSafe((r, col), board):
            ...
            backtrack_NQueens(board, solutions, queens-1, col+1)
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        if (input("Are you happy with your choice? (y/N): ").lower()[0] == 'y'):
            break 
    solutions = []
    backtrack_NQueens(board, solutions, N-1, 0)
